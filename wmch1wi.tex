...
\begin{defn}
Let $k$ be a $\s$-field and let $X$ be a $\s$-variety over $A$. Further let $F \subseteq A\{y_1, \ldots, y_n\}$ be a system of difference equations over $A$ with $X(B) = \VV_B(F) \fa B \in \sintk$.
...
We can now clarify what we meant after noting from Example \ref{A^n} the intuition that because $0$ is in any radical, mixed, difference ideal, it is not surprising that every $\s$-variety is a $\s$-subvariety of the one generated by $0$:

 \begin{lem}
Let $k$ be a $\s$-field. Then the maps $X \mapsto \I(X)$ and $\a \mapsto \V(\a)$ define inclusion-reversing bijections between the set of all $\s$-subvarieties of $\mathbb{A}^n$ and the radical, mixed ideals of $A\{y_1,\ldots,y_n\}$.
\begin{bew}
From Proposition \ref{I=F_m} we get that $\I(\V(\a)) = \a$ for all $\a \si A\{y_1,\ldots,y_n\}$ radial, mixed. Conversely, for a $\s$-variety $X = \VV(F) \subseteq \mathbb{A}^n$ we know that $\I(X) = \{F\}_m$, and $\VV(\I(X)) = \VV(\I(\VV(F))) \subseteq \VV(F) = X$,
...
\begin{cor}
  Let $X$ be a $\s$-variety over the integral $\s$-ring $A$. Then we get a bijection between the radical, mixed $\s$-ideals of $A\{X\}$ and the $\s$-subvarieties of $X$ via $X \supseteq Y \mapsto \{f \in A\{X\} \mid f(b) = 0 \fa b \in Y(B), \fa B \in \sintk \} =: \I_{A\{X\}}(Y)$
\end{cor}
...
\begin{bew}
Let $B \in \sintk$, $b \in B^n$ be a solution of $F \subseteq A\{y_1,\ldots,y_n\}$, i.e. $f(b) = 0 \fa f \in F$. Consider the mapping $\varphi: A\{y_1,\ldots,y_n\} \rightarrow B, y \mapsto b$ (``plugging in''). Then $F \subseteq $ ker$( \varphi) \si R$.
...
With Proposition \ref{bijsols} we see that it is a good idea to concentrate on $\sSpec(A\{X\})$ for a $\s$-variety $X$ over an integral $\s$-ring $A$.
...
\begin{defn}
Let $k$ be a $\s$-field, $X \subseteq \mathbb{A}^n,Y \subseteq \mathbb{A}^m$ $\s$-varieties over $A$. Then, a morphism of functors $f: X \rightarrow Y$ is called a morphism of $\s$-varieties over $A$ or $\s$-polynomial map if 
there exist $\s$-polynomials $f_1,\ldots,f_m \in A\{y_1,\ldots,y_n\}$ such that $f(b) = (f_1(b),\ldots,f_m(b))$ for all $b \in X(B)$ for all $B \in \sintk$.
\index{morphism of $\s$-varieties} \index{$\s$-polynomial map}
\end{defn}

\begin{ex}
For two $\s$-varieties $X \subseteq Y \subseteq \mathbb{A}^n_A$, over the integral $\s$-ring $A$, the inclusion mapping $\iota: X \hookrightarrow Y$ is a morphism of $\s$-varieties over $A$, since we can choose $f_1 = y_1, f_2 = y_2, \ldots, f_n = y_n$.
...
that $\varphi(b) = (f_1(b),\ldots,f_m(b))$ for all $b \in B, B \in \sintk$. Modulo $\I(X)$, these $f_i$ are unique:
 If there is $f_1', \ldots, f_m' \in A\{y_1,\ldots,y_n\}$ such that $f_i(b) = f'_i(b) \fa b \in B, B \in \sintk,$ and for all $i \in \underline{m}$,
...
\[ \varphi^*(h)(a) = h(\varphi(a)) \fa h \in A\{Y\}, a \in X(B), B \in \sintk \]
...
We thus get a contravariant functor $-^*$ from the category of difference varieties over $A$ to $\sringk$.
\end{rem}

\begin{prop}\label{dualisequiv}
Let $k$ be a $\s$-field. Then $-^*$ as defined in Remark \ref{dualmor} is an anti-equivalence between the categories of $\s$-varieties over $A$ and the subcategory of $\sringk$ of reduced, well-mixed, finitely $\s$-generated $\s$-overrings of $A$. 
...
and finitely $\s$-generated since $\s$-varieties are defined only on equations with finite many difference variables. This means that the functor $-^*$ from Remark \ref{dualmor} is also well defined on the category above.
It suffices to show that it is surjective on the skeleton of the categories and bijective on morphisms. 
Let $B$ be a finitely $\s$-generated, well-mixed and reduced $\s$-overring of $A$. We can then write $B \cong A\{y_1,\ldots,y_n\}/\a$, for an $\a \si A\{y_1,\ldots,y_n\}$ radical and mixed. The $\s$-variety $X = \VV(\a) \subseteq \mathbb{A}^n$
is then a preimage of the isomorphy class of $B$, since $\I(X) = \I(\VV(\a)) = \a$, and $\a = \{ \a \}_m$ because of Prop. \ref{I=F_m}. Thus, $B \cong A\{X\}$.
Now, for the morphisms: First, let $X,Y$ be $\s$-varieties over $A$ and $f,g \in \Hom(X,Y)$ with $f^* = g^*$. Then we know that for every $h \in A\{X\}$, and every $b \in B, B \in \sintk$ it holds that:
\[ h(f(b)) = f^*(h(b)) = g^*(h(b)) = h(g(b)) \]
In particular, $f(b) = g(b) \fa b \in B, B \in \sintk$, which means that $f = g$, and $-^*$ is injective. 
...
Then we define a morphism $f: X \rightarrow Y$ of $\s$-varieties over $A$ as follows: $f(b) := (f_1(b),\ldots,f_m(b)) \fa b \in B, B \in \sintk$. This is well-defined: Let $h \in \I(Y)$. Then, by definition, $h(y_1 + \I(Y),\ldots,y_n + I(Y)) = 0 + \I(Y)$.
This means however, that $h(f_1 + \I(X),\ldots,f_m + \I(X)) = 0 + \I(X)$, since $\varphi$ is a morphism of $\s$-overrings of $A$. But this in turn means, that $h(f(b)) = 0 \fa b \in X(B), B \in \sintk$, which means that $f$ maps indeed into $Y$, and $f^* = \varphi$.
\end{bew}
\end{prop}
...
We see thus how radical, mixed $\s$-ideals and the definition for $\s$-varieties as functors from $\sintk$, for an integral $\s$-ring $A$, as well as the topological space on $\sSpec(A\{X\})$ all fit together well. 
These are all in analogous relations to the case for perfect $\s$ ideals, where $\s$-varieties are defined from the category of $\s$-overfields of a $\s$-field $k$, and the topological space on $\Spec^\s(k\{X\})$ of $\s$-prime ideals.
The choice for the category $\sintk$ is better motivated by the following:
...
Let $X = \V(F)$ be a $\s$-variety in this sense, i.e., a functor from the category of overrings of $k$ which are perfectly $\s$-reduced and finitely $\s$-generated over $k$ to Set, the category of sets,
