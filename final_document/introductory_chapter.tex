...
\begin{defn}
Let $R$ be a $\s$-ring and $F \subset R$ be a $\s$-ideal of $R$. The smallest (with respect to inclusion) perfect $\s$-ideal containing $F$ is called the \emph{perfect closure of $F$}\index{perfect closure} and we will write $\{F\}$ to denote it. 
\end{defn}
...
We call this topology on $\Spec^\s(R)$ the \emph{Cohn topology}\index{Cohn topology}. A few interesting properties of the Cohn topology on $\Spec^\s(R)$ are presented in the folowing Proposition:
