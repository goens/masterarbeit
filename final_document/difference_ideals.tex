In this section we study difference ideals more closely. A special emphasis will be given to mixed and prime difference ideals which mimic in many ways the properties of and relationship between radical and prime ideals on rings.
...
which is very similar to Lemma 3.1 of \ref{levinmixed}, where the mixed closue is obtained this way.
\begin{lem}\label{lemshuffling}
Let $R$ be a $\s$-ring and $F \subseteq R$. Further let $F'~:=~{f\s(g)~\mid~fg~\in~F~\}$, and set $F^{\{1\}}:= [F']$, $F^{\{n\}}:= [{F^{\{n-1\}}}'] \fa n \in \NE$. Then
...
And hence $f + g, \s(f) \in F^{\{n\}} \subseteq \a$, as well as $fh \in F^{\{n\}} \subseteq \a$ for any $h \in R$. Furthermore, for $f, g \in R$ with $fg \in \a$ there also exists an $n \in \NE$ such that $fg \in F^{\{n\}}$. 
